...
from commons.arguments import parse_args
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False,model_pos=None,model_traj=None):
    epoch_loss_3d_pos = 0
...
def run_evaluation(actions, action_filter=None,model_pos=None,model_traj=None):
    errors_p1 = []
    errors_p2 = []
    errors_p3 = []
    errors_vel = []
    ...
    for action_key in actions.keys():
        ...
        e1, e2, e3, ev = evaluate(gen, action_key,model_pos,model_traj)
        errors_p1.append(e1)
        errors_p2.append(e2)
        errors_p3.append(e3)
...
if __name__ == '__main__':
    ...
    if not args.evaluate:
        ...
        if semi_supervised:
            cameras_semi, _, poses_semi_2d = fetch_data(subjects_semi, action_filter, parse_3d_poses=False)
        ...
        while epoch < args.epochs:
                ...
                if not args.no_eval:
                    ...
                    for cam, batch, batch_2d in test_generator.next_epoch():
                        ...
                        if semi_supervised:
                            ...
                            assert inputs_traj.shape[0] * inputs_traj.shape[1] == inputs_3d.shape[0] * inputs_3d.shape[
                                1]
                    ...
                    if semi_supervised:
                        for cam, _, batch_2d in semi_generator_eval.next_epoch():
                            ...
                            epoch_loss_2d_train_unlabeled_eval += reconstruction_semi.shape[0] * \
                                                                  reconstruction_semi.shape[
                                                                      1] \
            ...
            if args.export_training_curves and epoch > 3:
                ...
                if semi_supervised:
                    ...
                    plt.legend(
                        ['2d train labeled (eval)', '2d train unlabeled', '2d train unlabeled (eval)',
                         '2d valid (eval)'])
                    plt.ylabel('MPJPE (2D)')
                    plt.xlabel('Epoch')
                    plt.xlim((3, epoch))
    ...
    if args.render:
        ...
        if not args.by_subject:
            run_evaluation(all_actions, action_filter,model_pos,model_traj)
        else:
            for subject in all_actions_by_subject.keys():
                print('Evaluating on subject', subject)
                run_evaluation(all_actions_by_subject[subject], action_filter,model_pos,model_traj)
                print('')
